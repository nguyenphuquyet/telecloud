...
*   **Mã hóa AES-256-GCM**: Toàn bộ session Telegram và các cấu hình nhạy cảm (`api_id`, `api_hash`, `log_group_id`, `bot_tokens`) đều được mã hóa bằng AES-256-GCM trước khi lưu vào Cơ sở dữ liệu. Tiến trình di chuyển tự động (migration) sẽ diễn ra khi hệ thống khởi động.
    *   *SQLite*: Tự động tạo bản sao lưu `.pre-enc-<timestamp>.bak`.
    *   *MySQL / Postgres*: Yêu cầu biến `TELECLOUD_I_HAVE_BACKED_UP=1` để xác nhận an toàn trước khi chạy nâng cấp.
...
*   **Bảo mật WebDAV**: Áp dụng giới hạn tần suất (rate limit) nghiêm ngặt (5 lần thử / 15 phút cho mỗi địa chỉ IP) để chống dò mật khẩu (brute-force). Bộ nhớ cache xác thực ngắn (2 phút), sử dụng chuỗi mã hóa SHA-256 của mật khẩu thay vì lưu mật khẩu thô.
...
*   **Chính sách bảo mật nội dung (CSP)**: Trình duyệt mặc định chặn Cloudflare Web Analytics trừ khi bạn chủ động bật cấu hình `analytics_enabled=true`. Do frontend sử dụng Alpine.js, CSP bắt buộc vẫn cần hỗ trợ `'unsafe-inline'` và `'unsafe-eval'`.
...
*   **AES-256-GCM Encryption**: All Telegram sessions and sensitive configurations (`api_id`, `api_hash`, `log_group_id`, `bot_tokens`) are encrypted using AES-256-GCM at rest in the database.
    *   *SQLite*: Automatically creates a `.pre-enc-<timestamp>.bak` backup before migration.
    *   *MySQL / Postgres*: Requires the `TELECLOUD_I_HAVE_BACKED_UP=1` environment variable to confirm safety before performing the migration.
...
*   **WebDAV Authentication Hardening**: WebDAV enforces a strict rate limit of 5 failed attempts per 15 minutes per IP to prevent brute-force attacks. The auth cache expires in 2 minutes, using a SHA-256 hash of the password rather than plaintext.
...
*   **Content Security Policy (CSP)**: Excludes Cloudflare Web Analytics by default unless `analytics_enabled=true` is set. The policy permits `'unsafe-inline'` and `'unsafe-eval'` because the frontend leverages Alpine.js.
